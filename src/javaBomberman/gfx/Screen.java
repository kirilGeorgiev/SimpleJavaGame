...
public class Screen {
	...
	public static final byte BITS_MIRROR_X = 0x01;
	public static final byte BITS_MIRROR_Y = 0x02;
	...
	// Main render loop video tutorial 3
	...
	public void render(int xPos, int yPos, int tile, int colour,int mirrorDir, int scale) {
		...
		boolean mirrorX = (mirrorDir & BITS_MIRROR_X)>0;
		boolean mirrorY = (mirrorDir & BITS_MIRROR_Y)>0;
		...
		for (int y = 0; y < 8; y++) {			
			...
			int yPixel = y+yPos +(y*scaleMap)- ((scaleMap<<3)/2);   // <<3 (umnojenie po 16/2 )
